...
Component({
  ...
  methods: {
    ...
    getImagesInfo (views) {
      ...
      const loadTask = []
      for (let i = 0; i < Math.ceil(imageList.length / MAX_TASKS); i++) {
        loadTask.push(new Promise((resolve, reject) => {
          Promise.all(imageList.splice(i * MAX_TASKS, MAX_TASKS)).then(res => {
            resolve(res)
          }).catch(res => {
            reject(res)
          })
    ...
    startPainting () {
      ...
      this.ctx.draw(false, () => {
        ...
        //@issue: https://forum.cocos.com/t/setstoragesync-fail/63164/9
        // console.log('canvasdrawer_pic_cache:', this.cache, typeof this.cache)
        wx.setStorage({key:'canvasdrawer_pic_cache', data:this.cache})
